function [output] = bhmcmc(Y, T, delta, cgrid, Burnin, M, mat, Sigma_est, mu_p, nu, ws, vs)
...
if (mat) 
    ...
    if(isempty(nu))       
        ...
        [xx, fval] = fmincon(myobj, [1; 2.5], [], [], [], [], [1e-3; 2.5], [1e3; 4.5]);
        rho = xx(1);
        nu = xx(2);  
        ...
        nu = 2.5;
        [xx, fval] = fmincon(myobj, [1; nu], [], [], [], [], [1e-3; nu], [1e3; nu]);
        rho = xx(1);
        end
...
for iter = 1 : (M + Burnin)    
    ...
    if ~cgrid       
        for i = 1:n          
         ...
         Zvar = pinv(K + rn .* I);
         Zmean = (K + rn .* I) \ (rn .* Yfull + repmat(K * mu, 1, n)); 
         L = mychol(Zvar);
    ...
    if iter > Burnin
      ...
      % if (iter-Burnin) > 100
       %   rs = mean(rsOut((iter-Burnin-100):(iter-Burnin)));
      % end
    end
