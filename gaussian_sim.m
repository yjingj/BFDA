function [simdata] = gaussian_sim(t, n, sf, snr, nu, rho, dense)
%Inputs
% t: pooled grid
...
for i = 1 : p
    for j = 1 : p
    D(i, j) = abs(t(i) - t(j));
    C(i, j) = Matern(D(i, j), rho, nu, sf^2); %degree of Matern Covariance Function is nu = 2.5  
    end
end

%% Stationary Gaussian data on common grid

mut = 3*sin(t*4); % True signal mean on pooled grid
