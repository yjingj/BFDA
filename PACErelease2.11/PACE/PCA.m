...
function [no_opt,sigma,lambda,phi,eigen,xi_est,xi_var,mu,muDense,bw_mu,xcov,bw_xcov, xcovfit, AIC,BIC,FVE,y_pred, y_predOrig, y_predDense, out1,out21,...
	  y,t, regular, rho_opt, sigmanew, mucopy, phicopy, eigencopy, out1copy, out21copy, xcovcopy, xcovfitcopy, xcorr, ops]... 
         =PCA(y,t,bwmu,bwmu_gcv, bwxcov,bwxcov_gcv,ntest1,ngrid1,selection_k, FVE_threshold, maxk,...
              control,regular,error,ngrid,method,shrink,newdata,kernel, numBins,yname, screePlot, designPlot, corrPlot, rho, verbose, xcov)
...
if error==1
    ...
    if invalid==0
      [xi_est, xi_var, lambda, phi, eigen, no_opt, xcovfit,y_predOrig, rho_opt, sigmanew]=pc_est(y, t, mu, xcov, sigma, no_opt, error, method, shrink, out1, out21,regular,rho, verbose);
    else 
...
if invalid==0
    y_pred = repmat(mu, ncohort,1)+xi_est*phi';
    y_pred = num2cell(y_pred,2)';
    y_predDense = repmat(muDense, ncohort,1)+xi_est*eigen';
