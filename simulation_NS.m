...
%%
GausData = gaussian_simNS(t, n, sf, snr, nu, rho, dense);
...
b = 1; cgrid = 2; delta = 30; 

bs = 5;
BHM_cgrid = bhmcmc_fix_rho(Y_cgrid, T_cgrid, nu, delta, cgrid, b, bs,  Burnin, M); 
...
param_X = setOptions('regular', cgrid,'FVE_threshold',0.99,'corrPlot',0,'rho',-1); 
...
BHM_sparse = bhmcmc_fix_rho(Y, T, nu, delta, cgrid, b, bs, Burnin, M); 
...
%% Save outputs
save('StationaryGauss_NS.mat', 'GausData', 'BHM_cgrid', 'PCA_cgrid', 'Matern_cgrid',...
