function [step,ind,limwrd] = stepchk(step, cvec, deltac, limwrd, ind, ...
...
if any(stepi(active) < bot(active)-cvec(active))
  ...
  if dbgwrd
    fprintf(['Lower limit reached, old step, new step: ', ...
             '%10.4f, %10.4f\n'], step, stepnew);
  end
  step = stepnew;
...
if any(stepi(active) > top(active)-cvec(active))
  ...
  if dbgwrd
    fprintf(['Upper limit reached, old step, new step: ' ...
             '%10.4f, %10.4f\n'], step, stepnew);
  end
  step = stepnew;
