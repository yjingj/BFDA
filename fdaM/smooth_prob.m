...
for ivar=1:nvar
    for icurve=1:ncurve
        ...
        [logl, Dlogl] = loglfnpos(argvals, yi, basis, cvec0);
        ...
        hmat = loglhesspos(argvals, yi, basis, cvec0);
        if lambda > 0
            hmat = hmat + 2.*Kmat;
        end
        ...
        for iter = 1:iterlim
            ...
            for stepiter = 1:STEPMAX
                ...
                [logl, Dlogl] = loglfnpos(argvals, yi, basis, cvecnew);
                Fstr.f  = -logl;
                gvecnew = -Dlogl;
                if lambda > 0
            ...
            %  compute the Hessian
            hmat = loglhesspos(argvals, yi, basis, cvec);
            if lambda > 0
                hmat = hmat + 2.*Kmat;
            end
...
function [logl, Dlogl] = loglfnpos(argvals, yi, basis, cvec)
N = length(argvals);
...
function D2logl = loglhesspos(argvals, yi, basis, cvec)
N = length(argvals);
nbasis  = getnbasis(basis);
phimat  = getbasismatrix(argvals, basis);
Wvec    = phimat*cvec;
